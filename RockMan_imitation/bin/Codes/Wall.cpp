...
HRESULT CWall::Initialize()
{
	...
	D3DXMatrixScaling(&plat[0].matScale, 2.5f, 1.5f, 1);
	D3DXMatrixTranslation(&plat[0].matTrans, 650, -20, 0);
	plat[0].matWorld = plat[0].matScale * plat[0].matTrans;

	plat[0].vCenter = D3DXVECTOR3(0, 0, 0);
	plat[0].vPos = D3DXVECTOR3(0, 0, 0);

	D3DXMatrixScaling(&plat[1].matScale, 2.5, 1.5f, 1);
	...
	D3DXMatrixScaling(&plat[2].matScale, 2.5, 1.5f, 1);
...
{
	D3DXMatrixTranslation(&plat[0].matTrans, 650 + (*m_pvecScroll).x, -20 + (*m_pvecScroll).y, 0);
	plat[0].matWorld = plat[0].matScale * plat[0].matTrans;
	D3DXMatrixTranslation(&plat[1].matTrans, 650 + (*m_pvecScroll).x, 68 + (*m_pvecScroll).y, 0);
	plat[1].matWorld = plat[1].matScale * plat[1].matTrans;
	D3DXMatrixTranslation(&plat[2].matTrans, 650 + (*m_pvecScroll).x, 200 + (*m_pvecScroll).y, 0);
	plat[2].matWorld = plat[2].matScale * plat[2].matTrans;
	return S_OK;
}
